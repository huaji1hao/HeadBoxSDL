...
protected:
	...
	Scyjz14RawImageData(const Scyjz14RawImageData& src) = delete;

	// Do not allow assignment operator - which would cause problems with the internal data pointers.
	Scyjz14RawImageData& operator= (const Scyjz14RawImageData& src) = delete;

public:
	Scyjz14RawImageData(std::string filename)
